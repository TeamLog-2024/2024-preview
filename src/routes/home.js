...
const Particles = () => {
    ...
    } //초기위치를 랜덤으로 설정하고 50을 곱해서 적당한 거리로 위치 이동
...
const Planets = React.memo(({ radius, distance, rotationSpeed, orbitSpeed, textureURL, onClick}) => {
...
const Info = ({selectedPlanet}) => {
    ...
    return (
        <div className='info'>
        <p>행성 이름 : {planetName}</p>
        <p>질량 : {mass}</p>
        <p>지름 : {diameter}</p>
        <p>중력 : {gravity}</p>
        <p>표면 온도 : {temperature}</p>
        <p>공전 속도 : {orbital_velocity}</p>
        <p>위도 : {latitude_deg}</p>
        <p>경도 : {longtitude_deg}</p>
        <p>적위 : {declination}</p>
        <p>적경 : {right_ascension}</p>
        <p>지구와의 거리 : {earth_distance}</p>
        </div>
    );
}
